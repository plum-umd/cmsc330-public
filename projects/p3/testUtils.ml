...
let stmt_env : environment = [("x", Val_Int(1)); ("p", Val_Bool(false))]
let expr_env : environment = stmt_env @ [("y", Val_Int(6)); ("q", Val_Bool(true))]
...
let declaration_handler f =
  try
    let _ = f () in assert_failure "Expected DeclarationError, none received" with
  | DeclarationError(_) -> assert_succeed ()
  | ex -> assert_failure ("Got " ^ (Printexc.to_string ex) ^ ", expected DeclarationError")
...
type expected_exception = TypeExpect | DeclarationExpect | DivByZeroExpect
...
let assert_eval_fail env ast expect parse_fun eval_fun =
  ...
  let expect_function = begin match expect with 
    ...
    | DeclarationExpect -> declaration_handler
