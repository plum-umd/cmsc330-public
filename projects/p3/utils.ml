...
let parse_from_string (input : string) : stmt =
  let lex (input_string : string) : Lexing.lexbuf =
    let y = input_string ^ "\n" in
...
let parse_expr_from_string es = Parser.expr Lexer.tokens (Lexing.from_string @@ es ^ "\n")

let parse_stmt_from_string s = Parser.stmt Lexer.tokens (Lexing.from_string @@ s ^ "\n")
