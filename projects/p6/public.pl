...
test(public_product_empty) :-
	product([],1), !.

test(public_product_single) :-
	product([5],5), !.

test(public_product_primorial_1) :-
	product([2,3,5],30), !.

test(public_product_primorial_2) :-
	product([2,3,5,7],210), !.

test(public_product_unordered) :-
...
test(nodups_order_left) :-
	nodups([1,3,2,3,1],[1,3,2]), !.

test(nodups_order_right) :-
...
test(eval_stmt_declare) :-
	interpret_stmt("int x; bool p;",[],[],Env),
	opsem:lookup(Env,x,0),
	opsem:lookup(Env,p,false), !.

test(eval_stmt_assign) :-
	interpret_stmt("x = 1; p = true; x = 2;",[x-int,p-bool],[x-0,p-false],Env),
	opsem:lookup(Env,x,2),
	opsem:lookup(Env,p,true), !.

test(eval_stmt_while) :-
	interpret_stmt("int x; int n; x = 1; n = 4; while (0 < n) {x = x * n; n = n + (-1);}",[],[],Env),
	opsem:lookup(Env,x,24),
	opsem:lookup(Env,n,0), !.

test(eval_stmt_cond_then) :-
	interpret_stmt("if (x < x || p) {x = x + 1;} else {x = x + 2;}",[x-int,p-bool],[x-0,p-true],Env),
	opsem:lookup(Env,x,1),
	opsem:lookup(Env,p,true), !.

test(eval_stmt_cond_else) :-
...
test(accept_in_epsilon) :-
	nfa_suffix_epsilon(M),
	accept(M,[a,b,b,a]), !.

test(accept_in_noepsilon) :-
	nfa_third_last(M),
	accept(M,[b,a,b,b]), !.
